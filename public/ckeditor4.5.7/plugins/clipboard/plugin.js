...
(function() {
            ...
            editor.on(
                ...
                function(evt) {
                    ...
                    if (CKEDITOR.env.ie) {
                        ...
                        data = data.replace(/<\/(\w+)><div><br><\/div>$/, function(
                            match,
                            elementName
                        ) {
            ...
            editor.on('pasteDialog', function(evt) {
                ...
                if (!document.eNAEPExtensionId) {
                    ...
                    chrome.runtime.sendMessage(
                        document.eNAEPExtensionId, { getClipboardData: true },
                        null,
                        function(response) {
    ...
    var firePasteEvents = _.throttle(_firePasteEvents, 200);

    function _firePasteEvents(editor, data, withBeforePaste) {
        if (!data.type) {
            data.type = 'auto';
        }
    ...
    function initPasteClipboard(editor) {
        ...
        editor.getClipboardData = function(options, callback) {
            ...
            if (getClipboardDataDirectly() === false) {
                ...
                if (
                    beforePasteNotCanceled &&
                    editor.fire('pasteDialog', onDialogOpen)
                ) {
        ...
        function addPasteListenersToEditable() {
            ...
            editable.on(clipboard.mainPasteEvent, function(evt) {
                if (
                    clipboard.mainPasteEvent == 'beforepaste' &&
            ...
            editable.attachListener(
                CKEDITOR.env.ie ? editable : editor.document.getDocumentElement(),
                'mouseup',
                function() {
        ...
        function fixCut() {
            ...
            if (
                sel.getType() == CKEDITOR.SELECTION_ELEMENT &&
        ...
        function getClipboardDataByPastebin(evt, callback) {
            ...
            if (CKEDITOR.env.webkit) {
                ...
                editable
                    .getAscendant(CKEDITOR.env.ie ? 'body' : 'html', 1)
                    .append(pastebin);
            }
            ...
            if (isEditingHost) {
                ...
                pastebin.setStyle(
                    editor.config.contentsLangDirection == 'ltr' ? 'left' : 'right',
                    '-1000px'
                );
            }
        ...
        function getClipboardDataDirectly() {
            ...
            if (
                editor.editable().fire(clipboard.mainPasteEvent) &&
                !execIECommand('paste')
            ) {
                focusManager.unlock();
                return false;
        ...
        function pasteDataFromClipboard(evt) {
            ...
            if (
                beforePasteNotCanceled &&
                clipboard.canClipboardApiBeTrusted(eventData.dataTransfer, editor)
            ) {
                evt.data.preventDefault();
                setTimeout(function() {
                    if (!editor.readOnly) {
        ...
        function stateFromNamedCommand(command) {
            ...
            var sel = editor.getSelection(),
                ...
                selectionIsEmpty =
                sel.getType() == CKEDITOR.SELECTION_NONE ||
        ...
        editor.on('contentDom', function() {
            ...
            editable.attachListener(
                ...
                function() {
                    ...
                    var dragRange = (clipboard.dragRange = editor
                        .getSelection()
                        .getRanges()[0]);

                    // Store number of children, so we can later tell if any text node was split on drop. (#13011, #13447)
                    if (CKEDITOR.env.ie && CKEDITOR.env.version < 10) {
                        clipboard.dragStartContainerChildCount = dragRange ?
                            getContainerChildCount(dragRange.startContainer) :
                            null;
                        clipboard.dragEndContainerChildCount = dragRange ?
                            getContainerChildCount(dragRange.endContainer) :
                            null;
                    }
                },
                null,
            ...
            editable.attachListener(
                ...
                function(evt) {
                    ...
                    if (
                        readOnly &&
                        !(target.type == CKEDITOR.NODE_ELEMENT && target.is('html'))
                    ) {
                        return;
                    }
            ...
            editable.attachListener(
                ...
                function(evt) {
                    ...
                    if (
                        dataTransfer.getTransferType(editor) ==
            ...
            function crossEditorDrop(dragRange, dropRange, dataTransfer) {
                ...
                dropRange.select();
                firePasteEvents(
                    editor, { dataTransfer: dataTransfer, method: 'drop' },
                    1
                );
    ...
    CKEDITOR.plugins.clipboard = {
        ...
        canClipboardApiBeTrusted: function(dataTransfer, editor) {
            ...
            if (
                dataTransfer.getTransferType(editor) != CKEDITOR.DATA_TRANSFER_EXTERNAL
            ) {
                return true;
            }
         ...
         */
        fixSplitNodesAfterDrop: function(
            dragRange,
            dropRange,
            ...
            function handleContainer(dragContainer, dropContainer, preChildCount) {
                ...
                if (
                    dragElement.equals(dropContainer) &&
                    preChildCount != dropContainer.getChildCount()
                ) {
                    applyFix(dropRange);
                    return true;
                }
            ...
            function applyFix(dropRange) {
                var nodeBefore = dropRange.startContainer.getChild(
                        dropRange.startOffset - 1
                    ),
                    nodeAfter = dropRange.startContainer.getChild(dropRange.startOffset);

                if (
                    nodeBefore &&
                    nodeBefore.type == CKEDITOR.NODE_TEXT &&
                    nodeAfter &&
                    nodeAfter.type == CKEDITOR.NODE_TEXT
                ) {
        ...
        isDropRangeAffectedByDragRange: function(dragRange, dropRange) {
            ...
            //       ^ ^
            if (
                dragRange.endContainer.equals(dropContainer) &&
                dragRange.endOffset <= dropOffset
            ) {
                return true;
            }
        ...
        internalDrop: function(dragRange, dropRange, dataTransfer, editor) {
            ...
            isDropRangeAffected = this.isDropRangeAffectedByDragRange(
                dragRange,
                dropRange
            );
            ...
            firePasteEvents(
                editor, { dataTransfer: dataTransfer, method: 'drop', range: dropRange },
                1
            );
        ...
        getRangeAtDropPosition: function(dropEvt, editor) {
            ...
            if (document.caretRangeFromPoint) {
                ...
                range.setStart(
                    CKEDITOR.dom.node($range.startContainer),
                    $range.startOffset
                );
                range.collapse(true);
    ...
    var clipboardIdDataType = CKEDITOR.plugins.clipboard
     ...
     */
    CKEDITOR.plugins.clipboard.dataTransfer = function(
        nativeDataTransfer,
        editor
    ) {
        if (nativeDataTransfer) {
