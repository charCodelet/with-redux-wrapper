...
(function() {
    ...
    function buildDialog(editor) {
        var element = CKEDITOR.dom.element.createFromHtml(
            CKEDITOR.addTemplate('dialog', templateSource).output({
                ...
                baseFloatZIndex: CKEDITOR.config.baseFloatZIndex + 20, //1,
        ...
        //Ensure dragging off so dialog buttons work (draggable block touch click events),
        //only turn drag on when attemtping to drag (click on title) and when done make sure off (mouseUpHandler)
        // CKEDITOR.dialog.dragRoot = (element.$);  //need realiable access too
        // $(CKEDITOR.dialog.dragRoot).draggable({ disabled: false, enable: true, scroll: false});
    ...
    CKEDITOR.dialog = function(editor, dialogName) {
        ...
        function keydownHandler(evt) {
            ...
            if (keystroke == 9 || keystroke == CKEDITOR.SHIFT + 9) {
                ...
                if (!target.is('a', 'button', 'select', 'textarea') &&
                ...
                $('.cke_button__checkspell').removeClass(
                    'cke_button__checkspell_active'
                );
            } else {
                return;
            }
    ...
    CKEDITOR.dialog.prototype = {
        ...
        layout: function() {
            ...
            dialogSize.height = elBR.height;
            dialogSize.width = elBR.width;
         ...
         */
        hide: function() {
            // Resetting the newBlock flag here since we now know the dialog
            // was opened in this block.
            if (
                this.parts.title.$.textContent === 'Spell Check' ||
                this.parts.title.$.textContent === 'Thesaurus'
            ) {
                this._.editor.newBlock = false;
            }
            ...
            var tabId = 'cke_' + contents.id + '_' + CKEDITOR.tools.getNextNumber(),
                tab = CKEDITOR.dom.element.createFromHtml(
                    [
                        ...
                        titleHtml, !!contents.hidden ? ' style="display:none"' : '',
                        ' id="',
                        tabId,
                        '"',
    ...
    function initDragAndDrop(dialog) {
        ...
        function mouseMoveHandler(evt) {
            ...
            // console.log("zoom = " + getZoomPercent() + "viewPaneSize.width = " + viewPaneSize.width + "  viewPaneSize.height = " + viewPaneSize.height);

            //update viewPaneSize based on current window zoomPercent
            viewPaneSize.width = CKEDITOR.dialog.paneWidthWithZoom;
            viewPaneSize.height = CKEDITOR.dialog.paneHeightWithZoom;
            dialogSize.width = dialogSize.width * CKEDITOR.dialog.zoomPercent;
            dialogSize.height = dialogSize.height * CKEDITOR.dialog.zoomPercent;
            ...
            //do not move over toolbar
            if (realY < CKEDITOR.config.toolbarOffset) {
                realY = CKEDITOR.config.toolbarOffset;
            }
        ...
        function mouseUpHandler() {
            ...
            if (
                $(CKEDITOR.dialog.dragRoot) &&
                $(CKEDITOR.dialog.dragRoot).draggable &&
                typeof $(CKEDITOR.dialog.dragRoot).draggable === 'function'
            ) {
                $(CKEDITOR.dialog.dragRoot).draggable({ disabled: true });
            }
            ...
            setTimeout(function() {
                if (
                    ...
                    //reset allow for finger draggable
                    $(CKEDITOR.dialog.dragRoot).draggable({
                        disabled: false,
                        enable: true
                    });
        ...
        dialog.parts.title.on(
            'mousedown',
            function(evt) {
                var clearDialogFocusIndex = 0; //see focusList, need to send a changeFocus on a non select control to ensure all dropdwon selects are closed before dragging
                CKEDITOR.dialog.zoomOffsetX = getZoomOffsetX(dialog);
                CKEDITOR.dialog.zoomOffsetY = getZoomOffsetY(dialog);
                ...
                //set up draggable pane area based on zoom
                CKEDITOR.dialog.zoomPercent = getZoomPercent();
                var viewPaneSize = CKEDITOR.document.getWindow().getViewPaneSize();
                CKEDITOR.dialog.paneWidthWithZoom =
                    (viewPaneSize.width + CKEDITOR.dialog.zoomOffsetX) *
                    CKEDITOR.dialog.zoomPercent;
                CKEDITOR.dialog.paneHeightWithZoom =
                    (viewPaneSize.height + CKEDITOR.dialog.zoomOffsetY) *
                    CKEDITOR.dialog.zoomPercent;

                dialog.changeFocus(clearDialogFocusIndex); //close dropdown before dragging

                if ($(CKEDITOR.dialog.dragRoot) && $(CKEDITOR.dialog.dragRoot).draggable && typeof $(CKEDITOR.dialog.dragRoot).draggable === 'function') {
                    //allow for finger draggable
                    $(CKEDITOR.dialog.dragRoot).draggable({
                        disabled: false,
                        enable: true
                    });
    ...
    function getZoomLevel() {
        ...
        zoomLevel = '0'; //default
        if (zoomPercent >= 0.602 && zoomPercent <= 0.68) {
            zoomLevel = '0';
        } else if (zoomPercent <= 0.93) {
            zoomLevel = '1';
        } else if (zoomPercent <= 1.18) {
            zoomLevel = '2';
        } else if (zoomPercent <= 1.69) {
            zoomLevel = '3';
        }

        //   console.log("GETZOOMLEVEL = " + zoomLevel);
    ...
    function getZoomPercent() {
        ...
        var zoom = screenCssPixelRatio;
        return zoom;
    }

    function getZoomOffsets() {
        var dialogOffsets = {
            Desktop: {
                'Spell Check': {
                    '0': { x: 0, y: 0 },
    ...
    function getZoomOffsetX(dialog) {
        var zoomOffsetX = 295;
        var deviceProductId = localStorage.getItem('deviceProductId');
        if (deviceProductId === 'undefined' || deviceProductId === 'Unknown') {
            deviceProductId = 'Desktop';
        }
        var offsets = getZoomOffsets();
        var zoomLevel = getZoomLevel();

        if (dialog !== null && dialog !== undefined) {
            var currentDialogName = dialog.definition.title;
            var dialogNameIndex;

            if (currentDialogName !== null && currentDialogName !== undefined) {
                if (currentDialogName.indexOf('Spell Check Options') !== -1) {
                    dialogNameIndex = 'Options';
                } else if (currentDialogName.indexOf('Spell Check') !== -1) {
                    dialogNameIndex = 'Spell Check';
                } else if (currentDialogName.indexOf('Thesaurus') !== -1) {
                    dialogNameIndex = 'Thesaurus';
                }

                if (offsets[deviceProductId]) {
                    zoomOffsetX = offsets[deviceProductId][dialogNameIndex][zoomLevel].x;
                }
            }
        }
        return zoomOffsetX;
    ...
    function getZoomOffsetY(dialog) {
        var zoomOffsetY = 265;
        var deviceProductId = localStorage.getItem('deviceProductId');
        if (deviceProductId === 'undefined' || deviceProductId === 'Unknown') {
            deviceProductId = 'Desktop';
        ...
        if (dialog !== null && dialog !== undefined) {
            ...
            if (currentDialogName !== null && currentDialogName !== undefined) {
                ...
                if (offsets[deviceProductId]) {
                    zoomOffsetY = offsets[deviceProductId][dialogNameIndex][zoomLevel].y;
                }
            }
        }
        return zoomOffsetY;
        ...
        var mouseDownFn = CKEDITOR.tools.addFunction(function($event) {
            ...
            wrapperHeight =
                ...
                dialog.parts.contents.getSize(
                    'height', !(CKEDITOR.env.gecko || (CKEDITOR.env.ie && CKEDITOR.env.quirks))
                );
            wrapperWidth =
    ...
    function showCover(editor) {
        ...
        if (!coverElement) {
            var html = [
                ...
                '; z-index: ',
                baseFloatZIndex,
                '; top: 0px; left: 0px; ', !CKEDITOR.env.ie6Compat ?
        ...
        openDialog: function(dialogName, callback) {
            ...
            if (typeof dialogDefinitions == 'function') {
                ...
                if (
                    ...
                    (dialogName == 'checkspell' || dialogName == 'thesaurus')
                ) {
                    $('div.cke_editor_editor1_dialog').remove();
                    delete storedDialogs[dialogName];
                    delete storedDialogs['spelloptions'];
                }
