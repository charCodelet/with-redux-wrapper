...
var logConfig = {
	...
	eNaepEventSizeLimit: 0,  // max size of the events to hold in buffer, in KB
...
if (typeof logConfig == 'undefined') {
	...
	logMessage = function (message) {
		...
		if (typeof log != 'undefined') {
		    ...
		    if (logConfig.localLogging) alertLogError("Error: log object not created");
		}
	}
};
...
var getSelectionInfo = function (editor) {
	...
	if(logConfig.logKeyVerbose)
        selInfo.rightText = textSelected;
...
eventName2typeId = function (evtName) {
...
var textDiff = function (str1, str2) {
	...
	// we use the diffChars mode
	var diff = JsDiff.diffChars(str1, str2);
	var diffArray = [], counter = 1, keyRecord = {};
	...
	var sp = /[\s,\.\?\!\:\;]+/;
	var leftContext = str2.substring(0, keyRecord.pos - 1).split(sp).slice(-5).join(' ');
	var rightContext = str2.substring(keyRecord.pos - 1).split(sp).slice(0, 3).join(' ');
	var textContext = leftContext + "^" + rightContext;
	...
	CKEDITOR.plugins.add('keystrokelog', {
		...
		init: function (editor) {
			...
			function recordCommandAfter(event) {
				...
				if (event.data.commandData && event.data.commandData.from === "keystrokeHandler") {
					var eNaepName = eNAEPkeystrokes[event.data.name] || ("Keyboard_" + event.data.name.capitalizeFirstLetter());
					var extendedInfo = {
						//"eventTime": getCurTime(),
						"name": eNaepName,
					...
					//keystrokeManager.logit("COMMAND", extendedInfo);
					keystrokeManager.logit(eNaepName, logConfig.logEventVerbose ? extendedInfo : null);
				}
				keystrokeManager.save();
				// keystrokeManager.logdiff();
...
*/
		    ...
		    editor.on('menuItemCommand', function(evt) {
		        ...
		        if (evt.data.command.indexOf(scaytSuggestPrefix) === 0) {
		            editor.fire("spellchecker.event", {
		                "command": evt.data.command,
		                "event": "Misspellings_Corrected",
		                "newWord": evt.data.command.slice(scaytSuggestPrefix.length)
		            });
		        }
		        else {
		            // garyfeng: logging the save command

		            var eNaepName = "RightClick_" + evt.data.command.capitalizeFirstLetter();
		            var extendedInfo = {
		                //"eventTime": getCurTime(),
		                "name": eNaepName,
		                "command": evt.data.command
...
*/
			...
			editor.on('spellchecker.event', function(evt) {
				var eNaepName = "";
				var extendedInfo = {};
				switch (evt.data.event) {
					// These happen inside the spellchecker or thesaurus dialogs
					case "spellcheckerButtonClick":
						eNaepName = "Button_Spellchecker_" + evt.data.command.capitalizeFirstLetter();
						extendedInfo = {
							//"eventTime": getCurTime(),
							"name": eNaepName,
							"command": evt.data.command
	...
	KeystrokeManager.prototype = {
		...
		*/
		...
		logit: function (evt, extendedInfo) {
			...
			if (logConfig["eNaepAPI"]) {
			    ...
			    if (extendedInfo && typeof extendedInfo !== "string") {
			        ...
			        // Just a plain string to log, or no extended info
			        logEntry.extInfo = extendedInfo || null;
			    }

			    // convert eventName to typeId; get 999 if fails
				var typeId = eventName2typeId(evt);
				//var extInfo = JSON.stringifyOnce(logEntry);

				//if (typeof enaep == "undefined") {
				if (typeof CKEDITOR.enaepObservableEvent === "undefined") {
						//we don't see the enaep API
					logMessage(evt + "\teNAEPAPI(" + typeId + ", null, null, null, " +
						logEntry.timestamp + ", " + logEntry.extInfo + ")");

				} else {
					//enaep.observableEvent(typeId, blockId, itemId, studentId, eventTime, extendedInfo)
					// jgrant.
					//enaep.observableEvent(typeId, null, null, null, logEntry.timestamp, extInfo);
					// Create object containing event data.
					var oe = { type: typeId, time: logEntry.timestamp, extInfo: logEntry.extInfo };
		...
		logdiff: function() {
		    ...
		    var editor = this.editor, image = this.currentImage;
		    ...
		    if (!this.snapshots.length) // no snapshot, nothing to diff
		        return false;

		    if (this.snapshots.length < 2) // no snapshot, nothing to diff
		        return false;
    ...
    NativeEditingHandler.prototype = {
        ...
        onKeydown: function(evt) {
            this.lastKeyDownTime = evt.data.$.timeStamp;
            ...
            if (eNAEPEventName) {
                ...
                //this.keystrokeManager.logit(eNAEPEventName, extendedInfo);
                this.keystrokeManager.logit(eNAEPEventName, logConfig.logEventVerbose ? extendedInfo : undefined);
        ...
        onKeyup: function(evt) {
            ...
            // garyfeng: log it
            var keyTraveltime = this.keyEventsStack.getKeyTravelTime(keyCode, evt.data.$.timeStamp);
            var extendedInfo = {
                //"eventTime": getCurTime(evt.data.$.timeStamp),
                // "editor": editor.element.getId(),
                "name": evt.name,
        ...
        attachListeners: function() {
            ...
            editable.attachListener(editable, 'keydown', function(evt) {
                ...
                scrollInfo.method = "keyboard arrows";
                if (!KeystrokeManager.isNavigationKey(evt.data.getKey())) {
                    scrollInfo.ignoreScroll = true;
                }
            }, null, null, 999);
            ...
            editable.attachListener(editable.getDocument(), 'scroll', function (event) {
                 var keyLag = that.lastKeyDownTime
                    ? new Date().getTime() - that.lastKeyDownTime
                    : 1000;
                var keyLagExceeded = keyLag > 100;
                if (scrollInfo.ignoreScroll && !keyLagExceeded) {
                    // This is the case where non-navigational keystrokes triggered the scroll. We don't log those as observables since they are a "system action". Just return.
                    scrollInfo.ignoreScroll = false;
                    return;
                }
                scrollInfo.ignoreScroll = false;
                ...
                if (scrollInfo.method === "keyboard arrows" && keyLagExceeded) {
                ...
                var infoString = scrollInfo.zoomLevel + ", "
                    + scrollInfo.direction + ", "
                    + scrollInfo.method + ", Writing Panel, "
                    + Math.round(scrollInfo.hpos) + ", "
                    + Math.round(scrollInfo.vpos);
                that.keystrokeManager.logit("Vertical_Item_Scroll", infoString);
            });

            editable.attachListener(editable.getDocument(), 'swipe', function(/*event*/) {
                scrollInfo.method = "swipe";
            });

            editable.attachListener(editable.getDocument(), 'mousewheel', function(/*event*/) {
                scrollInfo.method = "mousewheel";
            });

            editable.attachListener(editable.getDocument(), 'mousedown', function (event) {
                if (event.data.$.which === 1) {
                    scrollInfo.method = "mouse/trackpad";
                }
            });

            editable.attachListener(editable.getDocument(), 'mouseup', function (event) {
                if (event.data.$.which === 1) {
	...
	KeyEventsStack.prototype = {
		...
		push: function (keyCode, timeStamp) {
			var length = this.stack.push({ keyCode: keyCode, inputs: 0, t: timeStamp });
