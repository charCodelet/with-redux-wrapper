...
(function () {
    ...
    var BuildCKEditorUndoStackAuditItems = function(index, currentImage) {
        var ckEditorUndoStackAuditItems = JSON.parse(localStorage.getItem("ckEditorUndoStackAuditItems"));
        if (!!ckEditorUndoStackAuditItems) {
            var localStorageImage = {
                bookmarks: currentImage.bookmarks,
                contents: currentImage.contents
            };
            ckEditorUndoStackAuditItems.index = index;
            ckEditorUndoStackAuditItems.currentImage = localStorageImage;
            localStorage.setItem("ckEditorUndoStackAuditItems", JSON.stringify(ckEditorUndoStackAuditItems));
        }
    };
    var SetUndoRedoFlagForCKEditorUndoStackAuditItems = function (hasUndo, hasRedo) {
        var ckEditorUndoStackAuditItems = JSON.parse(localStorage.getItem("ckEditorUndoStackAuditItems"));
        if (!!ckEditorUndoStackAuditItems) {
            ckEditorUndoStackAuditItems.hasUndo = hasUndo;
            ckEditorUndoStackAuditItems.hasRedo = hasRedo;
            localStorage.setItem("ckEditorUndoStackAuditItems", JSON.stringify(ckEditorUndoStackAuditItems));
        }
    };

    var IsFirstCallForCurrentItem = function() {
        if ((typeof (Storage) !== "undefined")) {
            if (!localStorage.getItem("ckEditorUndoStackAuditItems") || !localStorage.getItem("ckEditorUndoStack"))
                return true;
            var tmpCkEditorUndoStackAuditItems = JSON.parse(localStorage.getItem("ckEditorUndoStackAuditItems"));
            if (!tmpCkEditorUndoStackAuditItems.accessionNumber)
                tmpCkEditorUndoStackAuditItems.accessionNumber = "0";
            if (tmpCkEditorUndoStackAuditItems.accessionNumber !== CKEDITOR.currentAssesment)
                return true;

            //for some reason, there are 3 entries of snapshot for the first time. Checking that as well:
            var tmpCkEditorUndoStack = JSON.parse(localStorage.getItem("ckEditorUndoStack"));
            if (tmpCkEditorUndoStack.length === 3 && tmpCkEditorUndoStack[0].bookmarks.length === 0 && tmpCkEditorUndoStack[1].bookmarks.length === 0 && tmpCkEditorUndoStack[2].bookmarks.length === 0) {
                return true;
            }
        }
        return false;
    ...
    UndoManager.prototype = {
        ...
        reset: function () {
            ...
            //Recall Undo Stack from Local Storage:
            if ((typeof (Storage) !== "undefined") && (!!localStorage.getItem("ckEditorUndoStack"))) {
                var editor = this.editor;
                var ckEditorUndoStack = JSON.parse(localStorage.getItem("ckEditorUndoStack"));
                var ckEditorUndoStackAuditItems = JSON.parse(localStorage.getItem("ckEditorUndoStackAuditItems")); 

                //clean if different Accession Number (navigating to another Writing Editor)
                if(IsFirstCallForCurrentItem()) {
                    localStorage.removeItem("ckEditorUndoStack");
                    localStorage.removeItem("ckEditorUndoStackAuditItems");
                } else {
                    if (ckEditorUndoStack.length > 0) {
                        $.each(ckEditorUndoStack, function (index, value) {
                            var image = new Image(editor);
                            image.bookmarks = value.bookmarks;
                            image.contents = value.contents;
                            snapshots.push(image);
                        });

                        this.hasUndo = !!ckEditorUndoStackAuditItems.hasUndo;
                        this.hasRedo = !!ckEditorUndoStackAuditItems.hasRedo;

                        this.index = ckEditorUndoStackAuditItems.index;
                        if (!!ckEditorUndoStackAuditItems.currentImage) {
                            this.currentImage = new Image(editor);
                            this.currentImage.bookmarks = ckEditorUndoStackAuditItems.currentImage.bookmarks;
                            this.currentImage.contents = ckEditorUndoStackAuditItems.currentImage.contents;
                        }
                    }
                }
            }
        ...
        refreshState: function () {
            ...
            SetUndoRedoFlagForCKEditorUndoStackAuditItems(this.hasUndo, this.hasRedo);
            // Reset typing
            this.resetType();
            this.onChange();
        },
        ...
        save: function (onContentOnly, image, autoFireChange) {
            ...
            if (this.currentImage) {
                var regex = /<br><\/p>$/g;
                var currentImageContentsWOEndingBR = this.currentImage.contents.replace(regex, '') + '</p>';
                if (currentImageContentsWOEndingBR === image.contents)
                    return false;
               ...
               if (image.contents === this.currentImage.contents)
                    return false;              
            }
            ...
            //Copy the in-memory Undo stack snapshot into Local Storage
            if ((typeof (Storage) !== "undefined")) {
                var ckEditorUndoStack = [], localStorageImage = {};
                $.each(snapshots, function (index, value) {
                    localStorageImage = {
                        bookmarks : value.bookmarks,
                        contents : value.contents 
                    };
                    ckEditorUndoStack.push(localStorageImage);
                });
                localStorage.removeItem("ckEditorUndoStack");
                localStorage.setItem("ckEditorUndoStack", JSON.stringify(ckEditorUndoStack));
                var ckEditorUndoStackAuditItems = {
                    index: this.index,
                    accessionNumber: CKEDITOR.currentAssesment,
                    currentImage: localStorageImage,
        ...
        restoreImage: function (image) {
            ...
            BuildCKEditorUndoStackAuditItems(this.index, this.currentImage);
        ...
        undo: function () {
            if (this.undoable()) {
                ...
                if (image) {
                    ...
                    var sel = this.editor.getSelection(1);
                    var currentPath = this.editor.elementPath();
                    this.editor.fire('selectionChange', { selection: sel, path: currentPath });
                    return true;
                }
