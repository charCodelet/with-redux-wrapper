...
(function() {
    ...
    CKEDITOR.skin = {
        ...
        /**
         * ronmorrill add - need this to allow changing themes dynamically.
         * Disables any previous skins' stylesheets, and resets flag to allow reloading skin-specific CSS
         *
         */
        reset: function() {
            cssLoaded.editor = 0;
            // Disable any style sheets with 'skins' in them, since we will add new ones for the new skin
            var stylesLength = CKEDITOR.document.$.styleSheets.length;
            for (var i = 0; i < stylesLength; i++) {
                var styleSheet = CKEDITOR.document.$.styleSheets[i];
                if (styleSheet && !styleSheet.disabled && styleSheet.href && styleSheet.href.indexOf("skins") !== -1) {
                    console.log("STYLESHEET DISABLED " + styleSheet.href + " at index " + i.toString());
        ...
        reloadIcons: function() {
            ...
            this.icons = {}; // forces the icons to reload, via addIcon
            var iconNames = ('bold,copy,cut,LargeFont,MediumFont,SmallFont,indent,italic,outdent,paste,' +
                'redo,scayt,selectall,spellchecker,underline,thesaurus,undo').split(',');

            var iconsFolder = CKEDITOR.getUrl(CKEDITOR.skin.path() + 'icons/hidpi/');

            for (var i = 0; i < iconNames.length; i++) {
                CKEDITOR.skin.addIcon(iconNames[i], iconsFolder + iconNames[i] + '.png');
            }
        },
        ...
        var editor = evt.editor,
            showCallback = function(event) {
                ...
                if (!iframe.getById('cke_ui_color')) {
                    ...
                    if (color)
                        updateStylesheets([node], CKEDITOR.skin.chameleon(editor, 'panel'), [
                            [uiColorRegexp, color]
                        ]);
