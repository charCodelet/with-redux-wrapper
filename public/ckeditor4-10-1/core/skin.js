...
(function() {
  ...
  CKEDITOR.skin = {
    ...
    /**
     * ENAEP extension, to allow for dynamic theme changing by ensuring client has control
     * over loading css
     */
    getCssLoaded: function() {
      return cssLoaded;
    },
  ...
  function updateStylesheets(styleNodes, styleContent, replace) {
    ...
    for (var id = 0; id < styleNodes.length; id++) {
      if (CKEDITOR.env.webkit) {
        ...
        if (CKEDITOR.env.ie && CKEDITOR.env.version < 11) styleNodes[id].$.styleSheet.cssText += content;
