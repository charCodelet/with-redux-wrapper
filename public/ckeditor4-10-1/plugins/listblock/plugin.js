 ...
 *    see  OnLoad and  line 90 Proto:  (shortcutkeys support)  from 2017 #29692
 */
...
CKEDITOR.plugins.add('listblock', {
  ...
  onLoad: function() {
    var list = CKEDITOR.addTemplate('panel-list', '<ul role="presentation" class="cke_panel_list">{items}</ul>'),
      listItem = CKEDITOR.addTemplate(
        ...
        ' {onclick}="CKEDITOR.tools.callFunction({clickFn},\'{val}\'); return false;"' + // #188
          ' role="option">' +
          '{text}' +
          '<span class="cke_menu_shortcut">{shortcut}</span>' +
          '</a>' +
          '</li>'
      ),
    ...
    CKEDITOR.ui.listBlock = CKEDITOR.tools.createClass({
      ...
      proto: {
        add: function(value, html, title, shortcutKeys) {
          ...
          var data = {
            ...
            title: CKEDITOR.tools.htmlEncodeAttr(title),
            text: html || value,
        ...
        mark: function(value) {
          if (!this.multiSelect) this.unmarkAll();
        ...
        markFirstDisplayed: function() {
          ...
          this._.markFirstDisplayed(function() {
            if (!context.multiSelect) context.unmarkAll();
          });
        },
