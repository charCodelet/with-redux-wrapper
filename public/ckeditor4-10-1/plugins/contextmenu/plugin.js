...
CKEDITOR.plugins.add('contextmenu', {
  ...
  onLoad: function() {
    ...
    CKEDITOR.plugins.contextMenu = CKEDITOR.tools.createClass({
      ...
      proto: {
          ...
          element.on(
            ...
            function(event) {
              ...
              if (CKEDITOR.env.mac && CKEDITOR.env.webkit) {
                ...
                if (contentEditableParent && contentEditableParent.getAttribute('contenteditable') == 'false') editor.getSelection().fake(contentEditableParent);
              }
              ...
              CKEDITOR.tools.setTimeout(
                function() {
                  const item = this.items.map((item) => {
                    if (item.name === 'paste' && CKEDITOR.currentInstance.newBlock) {
                      item.className = 'cke_menubutton__paste cke_menubutton__disabled';
                    } else if (!CKEDITOR.currentInstance.newBlock) {
                      item.className = 'cke_menubutton__paste';
                    }
                  });

                  this.beforeOpen(offsetParent, null, offsetX, offsetY);
        ...
        /**
         * BeforeOpen the context menu is opened.  Give chance to collect any dynamic data (synonyms) needed
         * for context menu.
         *
         ...
         */
        beforeOpen: function(offsetParent, corner, offsetX, offsetY) {
          //save parms need for doOpen callback
          var plugin = CKEDITOR.plugins.contextMenu;
          plugin.doOpen_offsetParent = offsetParent;
          plugin.doOpen_corner = corner;
          plugin.doOpen_offsetX = offsetX;
          plugin.doOpen_offsetY = offsetY;
          plugin.doOpenContext = this;

          //fire event to go get any dynamic data for context menu  based on current selection
          this.editor.continueOnOpenCallback = this.doOpen;
          this.editor.fire('getSynonyms');
        },

        /**
         * doOpen - Callback to be used to finish the open process. (see beforeOpen)
         */
        doOpen: function(offsetParent, corner, offsetX, offsetY) {
          var plugin = CKEDITOR.plugins.contextMenu;
          var offsetParent = plugin.doOpen_offsetParent;
          var corner = plugin.doOpen_corner;
          var offsetX = plugin.doOpen_offsetX;
          var offsetY = plugin.doOpen_offsetY;
          var self = plugin.doOpenContext; //restore open context

          if (self.editor.config.enableContextMenu === false) {
            return;
          }
          self.editor.focus();
          ...
          self.editor.selectionChange(1);

          self.show(offsetParent, corner, offsetX, offsetY);
        },
        ...
        openHelper: function(offsetParent, corner, offsetX, offsetY) {}
      }
    });
  },
  ...
  beforeInit: function(editor) {
    ...
    if (editor.deviceProductId === 'SP3') {
      editor.setKeystroke(CKEDITOR.SHIFT + 35 /*-F10*/, 'contextMenu');
    } else {
      editor.setKeystroke(CKEDITOR.SHIFT + 33 /*-F10*/, 'contextMenu');
    }
  }
});
