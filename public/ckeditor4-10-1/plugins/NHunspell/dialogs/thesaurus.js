...
/// ENAPE UPDATES - 2020
///  (ljb) line 535  turn off close button not needed in new templates
///        line 465  (55,25,25) =>   widths: ['75%', '25%', '0%'],
///  Added method OnReplace and OnLookup to support new thesaurus controls
/// </summary>
            ...
            children: [
              ...
              },
              {
                ...
                onClick() {
                  ...
                  var plugin = CKEDITOR.plugins.NHunspell;
                  var lookedUp = dialog.getContentElement('thesaurus', 'lookedUp');
                  var targetWord = lookedUp.getValue();
                  var text = targetWord.toLowerCase();
                  ...
                  this.setupButtonState();
                  plugin.api.getThesaurus(targetWord, function(synonyms) {
                    dialog.synonyms = synonyms = compress(synonyms);
                ...
                onLookup(editor, targetWord) {
                  ...
                  if (text && plugin.lookupHistory.indexOf(text) === -1) {
                    ...
                    for (var i = 0, len = plugin.lookupHistory.length; i < len; i++) {
                      var text = plugin.lookupHistory[i];
                      //ljb lookedUp.add(CKEDITOR.tools.htmlEncode(text));
                    }
                  }

                  if (plugin.lookupHistory.length > 1) {
                    //ljb  var prev = dialog.getContentElement('thesaurus', 'prev');
                    //  prev.enable();
                  } else {
                    //ljb  var prev = dialog.getContentElement('thesaurus', 'prev');
                    //ljb  prev.disable();
                  ...
                  plugin.api.getThesaurus(targetWord, function(synonyms) {
                    ...
                    if (synonyms && synonyms.Meanings) {
                      for (var i = 0, len = synonyms.Meanings.length; i < len; i++) {
                        var meaning = synonyms.Meanings[i];
                        //ljb  lookedupList.add(meaning.Description);
                      }
                      //ljb lookedupList.setValue(synonyms.Meanings[0].Description);

                      //ljb  lookedupList.getElement().setStyle('visibility', 'visible');
                      //ljb  suggestReplace.getElement().setStyle('visibility', 'visible');

                      //ljb replaceWith.getElement().setStyle('visibility', 'visible');
                      //ljb buttonReplace.getElement().setStyle('visibility', 'visible');
                      //ljb replaceWithHtml.getElement().setStyle('visibility', 'visible');
                      // ljb noReccomedtations.getElement().setStyle('visibility', 'hidden');
                    } else {
                      //replaced hidden with disabled controls (Bug - 43780)
                      // ljb replaceWith.setup('');
                      // ljb noReccomedtations.getElement().setStyle('visibility', 'visible');
                    }
                  });
                },
                setupButtonState: function() {
          ...
          },
          {
            ...
            children: [
              ...
              },
              {
                ...
                onReplace: function(curEditor, origWord, replaceText) {
                  //var sel = editor.getSelection();
                  ...
                  //  var origWord = sel.getSelectedText();
                  curEditor.insertText(replaceText);

                  var extendedInfo = {
                    targetWord: origWord,
                    characterPosition: characterPosition,
                    replacementWord: replaceText
                  };
                  ...
                  var plugin = CKEDITOR.plugins.NHunspell;
                  plugin.api.logThesaurusReplace(origWord, replaceText);
              ...
              },
              {
                ...
                id: 'btCancel',
                label: 'Close',
                style: 'display:none; margin-left: -2px; padding-left: 3px; padding-right: 3px;',
                onClick: function(event) {
                  onClose();
