...
/// ENAPE UPDATES - 2020
///  (ljb) line 535  onFixTypo
//turn off close button not needed in new templates
  ...
  var ensureElementVisibility = function(wordElement) {
    ...
    var canFitBelow =
      $('.cke_contents > iframe')
        ...
        .find('body')
        .height() -
        contentsScrollingElement.scrollTop -
        wordInfo.top >
      0;
    ...
    console.info(
      '[NHUNSPELL-S] [ensureElementVisibility]\n  content frame container offset top: %d\n  content frame scroll top: %d\n' +
        ...
        '  element: %o\n' +
        '  dialog: %o\n',
      $contentsFrameContainer.offset().top,
      contentsScrollingElement.scrollTop,
      JSON.stringify(wordInfo),
      JSON.stringify(dialogInfo),
      JSON.stringify(wordOffset),
      $contentsFrameContainer[0],
      $wordElement[0],
      dialogElement[0]
    );
    console.info(
      '[NHUNSPELL-S] [ensureElementVisibility] obscured %s off page %s has scroll %s can fit below %s context above dialog %s',
      obscured,
      offPage,
      hasScroll,
      canFitBelow,
    ...
    if (!obscured && contextAboveDialog && canFitBelow) {
      ...
      var newX = wordOffset.left + wordInfo.width,
        newY = wordOffset.top - margin;
              ...
              },
              {
                ...
                children: [
                  ...
                  },
                  {
                    ...
                    onFixTypo: function(editor, typo, replaceText) {
                      var dialog = CKEDITOR.dialog.getCurrent();
                      ...
                      editor.insertHtml(replaceText, 'text');

                      var plugin = CKEDITOR.plugins.NHunspell;
                      //    plugin.api.logMisspellFromDialog(currentTypo, textInput.getValue());
                      plugin.api.logMisspellFromDialog(currentTypo, replaceText);
    ...
    onShow: function() {
      ...
      var callback = function() {
        ...
        if (ranges[0].collapsed) {
          // if nothing is selected on the editor
          ...
          if (plugin.isRevisitRequired) {
            ...
            if (
              firstNode.$.textContent !== nodeAtCursor.$.textContent ||
              firstNode.hasClass(NHUNSPELL_STYLE_TYPO) ||
              $(nodeAtCursor.$).find('.' + NHUNSPELL_STYLE_TYPO).length > 0
            ) {
          ...
          //if the text is selected on the editor
