...
/**
 *   ENAEP has updated this plugin from base ckeditor.   MODIFIED PLUGIN,
 *     migrated some changes from 4.5.7
 *     see line 372 - Bug 43711:Writing - Clear Clipboard when entering block - ID 33732 - removed item from template
 *         line 446 - new  editor.on('pasteDialog', function(evt) { ...}
 */
...
(function() {
    ...
    init: function(editor) {
      ...
      editor.on(
        ...
        function(evt) {
          ...
          //Bug 43711:Writing - Clear Clipboard when entering block - ID 33732
          if (editor.enterBlock) {
            return;
          }
      ...
      editor.on('pasteDialog', function(evt) {
        ...
        if (!document.eNAEPExtensionId) {
          setTimeout(function() {
            // Open default paste dialog.
            editor.openDialog('paste', evt.data);
          }, 0);
        } else {
          chrome.runtime.sendMessage(document.eNAEPExtensionId, { getClipboardData: true }, null, function(response) {
            //console.log('getClipboardData response: ' + JSON.stringify(response.data));
            var eventData = { type: 'auto', dataValue: response.data };
  ...
  function firePasteEvents(editor, data, withBeforePaste) {
    ...
    if (withBeforePaste) {
      ...
      // by other plugins.
      if (editor.fire('beforePaste', data) === false) return false; // Event canceled
    }
  ...
  function initPasteClipboard(editor) {
        ...
        if (editor._.forcePasteDialog && beforePasteNotCanceled && editor.fire('pasteDialog')) {
          ...
          editor.on('dialogHide', function(evt) {
            ...
            setTimeout(function() {
              ...
              if (!dialogCommited) callback(null);
            }, 10);
            // orig
            // // Notify even if user canceled dialog (clicked 'cancel', ESC, etc).
      ...
      if (clipboard.mainPasteEvent == 'beforepaste') {
        ...
        editable.on(
          ...
          function(evt) {
            ...
            if (evt.data && !evt.data.$.ctrlKey && !evt.data.$.shiftKey) preventBeforePasteEventNow();
          },
          null,
          null,
          0
    ...
    function createCutCopyCmd(type) {
      return {
        ...
        exec: function() {
          //Bug 43711:Writing - Clear Clipboard when entering block - ID 33732
          editor.enterBlock = false;

          // Attempts to execute the Cut and Copy operations.
          function tryToCutCopy(type) {
            if (CKEDITOR.env.ie) return execIECommand(type);
      ...
      return {
        ...
        exec: function(editor, data) {
          ...
          var cmd = this,
            ...
            keystroke = CKEDITOR.tools.keystrokeToString(editor.lang.common.keyboard, editor.getCommandKeystroke(this)),
            msg =
              typeof notification === 'string'
                ? notification
                : editor.lang.clipboard.pasteNotification.replace(/%1/, '<kbd aria-label="' + keystroke.aria + '">' + keystroke.display + '</kbd>'),
    ...
    function getClipboardDataByPastebin(evt, callback) {
      ...
      if (CKEDITOR.env.ie) sel.root.fire('selectionchange');
      ...
      if (CKEDITOR.env.webkit) {
        ...
        if (!editable.is('body')) {
          ...
          if (editable.getComputedStyle('position') != 'static') offsetParent = editable;
      ...
      if (CKEDITOR.env.safari) pastebin.setStyles(CKEDITOR.tools.cssVendorPrefix('user-select', 'text'));
      ...
      if (CKEDITOR.env.webkit || CKEDITOR.env.gecko) blurListener = editable.once('blur', cancel, null, null, -100);
    ...
    function pasteDataFromClipboard(evt) {
      ...
      if (beforePasteNotCanceled && clipboard.canClipboardApiBeTrusted(eventData.dataTransfer, editor)) {
        ...
        getClipboardDataByPastebin(evt, function(data) {
          // Clean up.
          eventData.dataValue = data.replace(/<span[^>]+data-cke-bookmark[^<]*?<\/span>/gi, '');
    ...
    function stateFromNamedCommand(command) {
      ...
      if (command == 'paste') return CKEDITOR.TRISTATE_OFF;
  ...
  function recogniseContentType(data) {
    if (CKEDITOR.env.webkit) {
      ...
      if (!data.match(/^[^<]*$/g) && !data.match(/^(<div><br( ?\/)?><\/div>|<div>[^<]*<\/div>)*$/gi)) return 'html';
  ...
  function initDragDrop(editor) {
    ...
    editor.on('contentDom', function() {
      ...
      editable.attachListener(dropTarget, 'dragover', function(evt) {
        ...
        if (CKEDITOR.env.ie && CKEDITOR.plugins.clipboard.isFileApiSupported && evt.data.$.dataTransfer.types.contains('Files')) {
          evt.data.preventDefault();
        }
      });
  ...
  CKEDITOR.plugins.clipboard = {
    ...
    fixSplitNodesAfterDrop: function(dragRange, dropRange, preDragStartContainerChildCount, preDragEndContainerChildCount) {
      ...
      if (typeof preDragEndContainerChildCount != 'number' || typeof preDragStartContainerChildCount != 'number') {
        return;
      }
      ...
      function applyFix(dropRange) {
        ...
        if (nodeBefore && nodeBefore.type == CKEDITOR.NODE_TEXT && nodeAfter && nodeAfter.type == CKEDITOR.NODE_TEXT) {
    ...
    isDropRangeAffectedByDragRange: function(dragRange, dropRange) {
      ...
      // " O [L A " " M A "
      //           ^       ^
      if (dragRange.startContainer.getParent().equals(dropContainer) && dragRange.startContainer.getIndex() < dropOffset) {
        return true;
      }
      ...
      // " O] L A " " M A "
      //           ^       ^
      if (dragRange.endContainer.getParent().equals(dropContainer) && dragRange.endContainer.getIndex() < dropOffset) {
        return true;
      }
    ...
    internalDrop: function(dragRange, dropRange, dataTransfer, editor) {
      ...
      if (CKEDITOR.env.ie && CKEDITOR.env.version < 10) {
        this.fixSplitNodesAfterDrop(dragRange, dropRange, clipboard.dragStartContainerChildCount, clipboard.dragEndContainerChildCount);
      }
      ...
      var startNode = dragBookmark.startNode,
        ...
        dropInsideDragRange =
          ...
          endNode && startNode.getPosition(dropNode) & CKEDITOR.POSITION_PRECEDING && endNode.getPosition(dropNode) & CKEDITOR.POSITION_FOLLOWING;
    ...
    getRangeAtDropPosition: function(dropEvt, editor) {
      ...
      if (dropEvt.data.testRange) return dropEvt.data.testRange;
      ...
      // IE 8 and all IEs if !defaultRange or external DnD.
        ...
        try {
          ...
          if (sucess) {
            ...
            else if (defaultRange && defaultRange.startContainer && !defaultRange.startContainer.equals(editor.editable())) {
              return defaultRange;
  ...
  CKEDITOR.plugins.clipboard.dataTransfer.prototype = {
    ...
    getData: function(type, getNative) {
      ...
      if (type == 'Text' && CKEDITOR.env.gecko && this.getFilesCount() && data.substring(0, 7) == 'file://') {
        data = '';
      }
