 ...
 *     ported over enape 2019 4.5.7 version (too many changes to merged)
 **/
...
(function() {
  ...
  toolbox.prototype.focus = function() {
    for (var t = 0, toolbar; (toolbar = this.toolbars[t++]); ) {
      for (var i = 0, item; (item = toolbar.items[i++]); ) {
        if (item.focus) {
          // Ron Morrill: added the IF to prevent setting focus to disabled buttons
  ...
  CKEDITOR.SimulatedTabKey = 10; //used to auto jump groupings when tabbing
  CKEDITOR.plugins.add('toolbar', {
    ...
    init: function(editor) {
      ...
      var itemKeystroke = function(item, keystroke) {
        ...
        toolbarTabGroupCycling = false;
        toolbarArrowGroupCycling = false;

        switch (keystroke) {
          case CKEDITOR.SimulatedTabKey: // TAB - performs an original tab action, used by real tab key processing to move to next grouping
          case CKEDITOR.SHIFT + CKEDITOR.SimulatedTabKey: // SHIFT + TAB
            // Cycle through the toolbars, starting from the one
            // closest to the current item.
            var backwards = keystroke !== CKEDITOR.SimulatedTabKey;
            while (item && (!toolbar || !toolbar.items.length)) {
              if (!backwards) {
              ...
              if (toolbar && toolbar.items.length) {
                item = toolbar.items[endFlag ? toolbar.items.length - 1 : 0];
                while (item && (!item.focus || !item.focus())) {
                  var prevItem = item;
          ...
          case 9: // TAB - tab in one group calls with SimulatedTabKey to jump to next grouping when at end of this grouping and tab is pressed again
            next = item;
            do {
              ...
              if (!next && toolbarTabGroupCycling) next = item.toolbar.items[0];
            } while (next && (!next.focus || !next.focus()));
            ...
            if (next) {
              ...
              // Send a auto TAB.
              itemKeystroke(item, CKEDITOR.SimulatedTabKey);
            }

            return false;

          case CKEDITOR.SHIFT + 9: // SHIFT + TAB moves "left" through current tab grouping when at start will call with 'CKEDITOR.SHIFT + CKEDITOR.SimulatedTabKey' to move to previous grouping
            next = item;
            do {
              ...
              if (!next && toolbarTabGroupCycling) next = item.toolbar.items[item.toolbar.items.length - 1];
            } while (next && (!next.focus || !next.focus()));
            ...
            if (!next) {
              ...
              //else {
              endFlag = 1;
              // Send a Auto SHIFT + TAB.
              itemKeystroke(item, CKEDITOR.SHIFT + CKEDITOR.SimulatedTabKey);
              endFlag = 0;
            }
          ...
          case rightKeyCode:
            ...
            do {
              ...
              // If it's the last item, cycle to the first one.
              if (!next && toolbarArrowGroupCycling) next = item.toolbar.items[0];
            } while (next && (!next.focus || !next.focus()));
          ...
          case leftKeyCode:
            ...
            do {
              ...
              // If it's the first item, cycle to the last one.
              if (!next && toolbarArrowGroupCycling) next = item.toolbar.items[item.toolbar.items.length - 1];
            } while (next && (!next.focus || !next.focus()));
      ...
      editor.on('uiSpace', function(event) {
        ...
        var output = [
          ...
          '</span>',
          '<span id="' + editor.ui.spaceId('toolbox') + '" class="cke_toolbox" role="group" aria-labelledby="',
          labelId,
        ...
        if (editor.config.toolbarCanCollapse && editor.elementMode != CKEDITOR.ELEMENT_MODE_INLINE) {
          ...
          output.push(
            '<a title="' +
              (expanded ? editor.lang.toolbar.toolbarCollapse : editor.lang.toolbar.toolbarExpand) +
              '" id="' +
      ...
      if (editor.deviceProductId === 'SP3') {
        //alt + (-F10), sp3
        editor.setKeystroke(CKEDITOR.ALT + 35, 'toolbarFocus');
      } else {
        //alt + (-F10), sp4
        editor.setKeystroke(CKEDITOR.ALT + 33, 'toolbarFocus');
      }
