 ...
 *      all Changes from 2017/2019 have been incorporated
 */
...
CKEDITOR.plugins.add('menu', {
  ...
  beforeInit: function(editor) {
    ...
    editor.addMenuItem = function(name, definition) {
      if (groupsOrder[definition.group]) menuItems[name] = new CKEDITOR.menuItem(this, name, definition);
    };
...
(function() {
  ...
  if (CKEDITOR.env.gecko && CKEDITOR.env.mac) menuItemSource += ' onkeypress="return false;"';
  ...
  CKEDITOR.menu = CKEDITOR.tools.createClass({
    ...
    $: function(editor, definition) {
      ...
      var panelDefinition = CKEDITOR.tools.extend({}, definition.panel, {
        css: [CKEDITOR.skin.getPath('editor'), CKEDITOR.config.contentsCss],
    ...
    _: {
        ...
        for (var i = 0; i < listeners.length; i++) {
          ...
          if (listenerItems) {
            for (var itemName in listenerItems) {
              ...
              // Ron Morrill: Change this check to only "if (item)" in order for the disabled command to still show in the menu, but in a disabled state
              if (item && (!item.command || this.editor.getCommand(item.command).state)) {
                if (itemName === 'paste') {
                  if (CKEDITOR.currentInstance.newBlock) {
                    item.state = CKEDITOR.TRISTATE_ON;
                    item.className = 'cke_menubutton__disabled';
                  } else {
                    item.state = CKEDITOR.TRISTATE_OFF;
      ...
      onClick: function(item) {
        ...
        if (item.onClick) item.onClick();
          ...
          // garyfeng:
          this.editor.fire('menuItemCommand', item);
          // garyfeng: end
      ...
      onHide: function() {
        ...
        this.removeAll(); //clear out context menu
      },

      calculateSubMenuOffsets: function(menu) {
        var offsetYBase = 146; //172;  //10 parent menu items * 24 menu item height - fudge factor
        var menuItemHeight = 18.25; //24;
        ...
        // calculatYoffset
        //   var offsetY = 73;
        var offsetY = offsetYBase - menu.items.length * menuItemHeight;

        menu.offsetX = 105;
        menu.offsetY = 242;
        //if (menu.items.length > 4) {
        //  menu.offsetY += 51;  //menu will display up bring it down to base of parent menu
        //}
      },
      showSubMenu: function(index) {
        //ensure menu is sorted so correct command is exectued
        if (!this.itemsSorted) {
        ...
        for (var subItemName in subItemDefs) {
          ...
          if (subItem) {
            ...
            if (subItem.state === CKEDITOR.TRISTATE_DISABLED) {
              subItem.className = 'cke_menubutton__disabled';
            }
            menu.add(subItem);
          }
        }
        ...
        //calculate the sub menu offsets
        this._.calculateSubMenuOffsets(menu);

        // Show the submenu.
        // This timeout is needed to give time for the sub-menu get
        // focus when JAWS is running. (#9844)
        setTimeout(function() {
          menu.show(element, 2, menu.offsetX, menu.offsetY);
        }, 0);
      }
    },
  ...
  CKEDITOR.menuItem = CKEDITOR.tools.createClass({
    ...
    proto: {
      render: function(menu, index, output) {
        ...
        if (this.role in { menuitemcheckbox: 1, menuitemradio: 1 }) ariaChecked = ' aria-checked="' + (state == CKEDITOR.TRISTATE_ON ? 'true' : 'false') + '"';
