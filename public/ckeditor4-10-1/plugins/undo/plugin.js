...
(function() {
  ...
  CKEDITOR.plugins.add('undo', {
    ...
    init: function(editor) {
      ...
      editor.setKeystroke([[keystrokes[0], 'undo'], [keystrokes[1], 'redo'], [keystrokes[2], 'redo']]);
      ...
      function recordCommand(event) {
        ...
        if (undoManager.enabled && event.data.command.canUndo !== false) undoManager.save();
      }
  ...
  UndoManager.prototype = {
    ...
    type: function(keyCode, strokesPerSnapshotExceeded) {
      ...
      strokesPerSnapshotExceeded = strokesPerSnapshotExceeded || strokesRecorded >= this.strokesLimit;
    ...
    save: function(onContentOnly, image, autoFireChange) {
      ...
      if (this.locked || editor.status != 'ready' || editor.mode != 'wysiwyg') return false;
      ...
      if (!this.locked) {
        if (dontUpdate) this.locked = { level: 1 };
          ...
          if (forceUpdate) update = true;
            ...
            // during this period.
            if (this.currentImage && this.currentImage.equalsContent(imageBefore)) update = imageBefore;
          }
  ...
  var Image = (CKEDITOR.plugins.undo.Image = function(editor, contentsOnly) {
    ...
    if (CKEDITOR.env.ie && contents) contents = contents.replace(/\s+data-cke-expando=".*?"/g, '');
  ...
  Image.prototype = {
    ...
    equalsSelection: function(otherImage) {
      ...
      if (bookmarksA || bookmarksB) {
        if (!bookmarksA || !bookmarksB || bookmarksA.length != bookmarksB.length) return false;
  ...
  NativeEditingHandler.prototype = {
    ...
    onKeyup: function(evt) {
      ...
      if (UndoManager.ieFunctionalKeysBug(keyCode) && this.lastKeydownImage && this.lastKeydownImage.equalsContent(new Image(undoManager.editor, true))) {
        return;
      }
    ...
    onNavigationKey: function(skipContentCompare) {
      ...
      if (skipContentCompare || !undoManager.save(true, null, false)) undoManager.updateSelection(new Image(undoManager.editor));
