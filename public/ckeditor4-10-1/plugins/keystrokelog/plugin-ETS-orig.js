...
var logConfig = {
  ...
  eNaepPostInterval: 16000,  // how many seconds between calls
...
var getSelectionInfo = function(editor) {
  // get the current selectionn, return a selInfo object

  var selInfo = {}, textSelected ="";
...
var eventName2typeIdTable = {
	...
	"Button_Underscore": 43,
	//"Button_Underline": 43,
	"Keyboard_Underscore": 44,
	"Menu_Underscore": 45,
	//"Menu_Underline": 45,
	"RightClick_Underscore": 46,
	"Button_Indent": 47,
	"Button_Outdent": 48,
	"Button_SpellCheck": 49,
	"Menu_SpellCheck": 50,
	"Button_Misspellings": 51,
...
( function() {
  ...
  eNAEPkeystrokes[CKEDITOR.CTRL + 85]= "Keyboard_Underscore";  // CTRL-u
	...
	CKEDITOR.plugins.add( 'keystrokelog', {
      ...
      function recordCommandAfter( event ) {
        ...
        var extendedInfo={
          ...
          "source": event.data.commandData?event.data.commandData.from : "notKeyboard"
        }
				//keystrokeManager.logit("COMMAND", extendedInfo);
        keystrokeManager.save();
        // keystrokeManager.logdiff();
...
//console.log(evt);
      ...
      */
      ...
      logit: function( evt, extendedInfo) {
        ...
        if (logConfig["formatUsingXML"]) {
          // using XML coding
          if (typeof extendedInfo != 'undefined') {
        ...
        if (logConfig["eNaepAPI"])  {
          ...
          if (typeof extendedInfo != 'undefined') {
            logEntry.extInfo= JSON.stringifyOnce(extendedInfo).encodeHTML();
            //consoleDebuging(logEntry.extInfo);
          } else {
            // no extendedInfo
            logEntry.extInfo=null;
          }

          // convert eventName to typeId; get 999 if fails
          var typeId = eventName2typeId(evt);
          var extInfo = JSON.stringifyOnce(logEntry).encodeHTML();

          if (typeof enaep== "undefined") {
            //we don't see the enape API
            logMessage(evt+"\teNAEPAPI("+typeId+", null, null, null, "+
              logEntry.timestamp+", "+extInfo+")")
            ...
            var oe = {type: typeId, block: null, item: null, student: null, time: logEntry.timestamp, eInfo: extInfo};
            // If the event array has reached 500KB, send the data.
            if (this.getSizeOf(gObsEvents) >= logConfig.eNaepEventSizeLimit) {
              this.sendEventData();
            }
            // Otherwise, push data to the event array.
            else {
              gObsEvents.push(oe);
            }
          }
    ...
    logdiff: function() {
      ...
      if (curTexts != prevTexts) {
        var diffs=textDiff(prevTexts, curTexts);
      } else {
	...
	NativeEditingHandler.prototype = {
      ...
      //// end garyfeng
			...
			if (extendedInfo && extendedInfo.ctrlKey == true) {
