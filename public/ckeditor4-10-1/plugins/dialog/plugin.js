...
(function() {
  ...
  function getPreviousVisibleTab() {
    ...
    for (var i = tabIndex - 1; i > tabIndex - length; i--) {
      if (isTabVisible.call(this, this._.tabIdList[i % length])) return this._.tabIdList[i % length];
    }
  ...
  function buildDialog(editor) {
    ...
    if (CKEDITOR.env.ie && !CKEDITOR.env.quirks && !CKEDITOR.env.edge) {
      var src = 'javascript:void(function(){' + encodeURIComponent('document.open();(' + CKEDITOR.tools.fixDomain + ')();document.close();') + '}())', // jshint ignore:line
        iframe = CKEDITOR.dom.element.createFromHtml('<iframe' + ' frameBorder="0"' + ' class="cke_iframe_shim"' + ' src="' + src + '"' + ' tabIndex="-1"' + '></iframe>');
  ...
  CKEDITOR.dialog = function(editor, dialogName) {
    ...
    if (editor._.removeDialogTabs && (tabsToRemove = editor._.removeDialogTabs[dialogName])) {
      for (i = 0; i < tabsToRemove.length; i++) definition.removeContents(tabsToRemove[i]);
    }
    ...
    if (definition.onCancel) {
      this.on('cancel', function(evt) {
        if (definition.onCancel.call(this, evt) === false) evt.data.hide = false;
      });
    }
    ...
    this.on('show', function() {
      ...
      if (CKEDITOR.env.gecko) dialogElement.on('keypress', keypressHandler, this);
    });
    this.on('hide', function() {
      dialogElement.removeListener('keydown', keydownHandler);
      if (CKEDITOR.env.gecko) dialogElement.removeListener('keypress', keypressHandler);
    ...
    for (i = 0; i < buttons.length; i++) this._.buttons[buttons[i].id] = buttons[i];
  ...
  CKEDITOR.dialog.prototype = {
    ...
    resize: (function() {
      return function(width, height) {
        if (this._.contentSize && this._.contentSize.width == width && this._.contentSize.height == height) return;
    ...
    show: function() {
      ...
      for (var i in definition.contents) {
        ...
        for (var j in this._.contents[content.id]) {
          ...
          if (elem.type == 'hbox' || elem.type == 'vbox' || !elem.getInputElement()) continue;

          if (elem.requiredContent && !this._.editor.activeFilter.check(elem.requiredContent)) elem.disable();
          else {
            elem.enable();
            enableElements++;
          }
        ...
        if (!enableElements || (requiredContent && !this._.editor.activeFilter.check(requiredContent))) tab[0].addClass('cke_dialog_tab_disabled');
    ...
    addPage: function(contents) {
      if (contents.requiredContent && !this._.editor.filter.check(contents.requiredContent)) return;
      ...
      while ((cursor = children.shift())) {
        ...
        if (!cursor.notAllowed && cursor.type != 'hbox' && cursor.type != 'vbox') enabledFields++;
    ...
    selectPage: function(id) {
      ...
      if (this.fire('selectPage', { page: id, currentPage: this._.currentTabId }) === false) return;
    ...
    addFocusable: function(element, index) {
      if (typeof index == 'undefined') {
        ...
        for (var i = index + 1; i < this._.focusList.length; i++) this._.focusList[i].focusIndex++;
      }
    },
  ...
  CKEDITOR.tools.extend(CKEDITOR.dialog, {
    ...
    add: function(name, dialogDefinition) {
      ...
      if (!this._.dialogDefinitions[name] || typeof dialogDefinition == 'function') this._.dialogDefinitions[name] = dialogDefinition;
    },
  ...
  var definitionObject = function(dialog, dialogDefinition) {
    ...
    for (var i = 0, content; (content = contents[i]); i++) contents[i] = content && new contentObject(dialog, content);
  ...
  var unregisterAccessKey = function(obj) {
    for (var i in accessKeyProcessors) {
      ...
      for (var j = list.length - 1; j >= 0; j--) {
        if (list[j].dialog == obj || list[j].uiElement == obj) list.splice(j, 1);
      }
  ...
  var tabAccessKeyUp = function(dialog, key) {
    if (dialog._.accessKeyMap[key]) dialog.selectPage(dialog._.accessKeyMap[key]);
  };
      ...
      functions: function() {
        ...
        return function() {
          ...
          for (i = 0; i < functions.length; i++) {
            if (relation == CKEDITOR.VALIDATE_AND) passed = passed && functions[i](value);
