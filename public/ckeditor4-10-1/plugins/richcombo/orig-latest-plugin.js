 ...
 *     Merged all 2017 and 2019 updates
 *     see line 29  - removed item from template
 *         line 142 - removed focus (see comment)
 *         line 272 - new onShow
 *         line 372 - menu has been closed, clear openMenu so no attempt is made to close againew show
 *         line 439 - add - handling shortcuts
 */
    ...
    proto: {
      ...
      render: function(editor, output) {
        ...
        var instance = {
          id: id,
          combo: this,
          // Ron Morrill: prevent tabbing to the combo items. Only toolbar is available for keyboard navigation
          //focus: function() {
        ...
        function updateState() {
          ...
          if (this.getState() == CKEDITOR.TRISTATE_ON) return;

          var state = this.modes[editor.mode] ? CKEDITOR.TRISTATE_OFF : CKEDITOR.TRISTATE_DISABLED;

          if (editor.readOnly && !this.readOnly) state = CKEDITOR.TRISTATE_DISABLED;

          this.setState(state);
          this.setValue('');

          // Let plugin to disable button.
          if (state != CKEDITOR.TRISTATE_DISABLED && this.refresh) this.refresh();
        }
      ...
      createPanel: function(editor) {
        ...
        panel.onShow = function() {
          // In order to reposition the dropdowns before showing them
          // we need to hide, reposition, then show.
          me._.panel.element.setStyle('display', 'none');
          this.element.addClass(namedPanelCls);

          //  Object {top: 122.40139770507812, left: 31.27220790809504}
          var btnPosition = $(me._.panel.element.$).position();
          if (me.label === 'Tools') {
            // Change the left position for the Tools dropdown so it
            // hangs to the right.
            btnPosition.left -= 60;
          }
          ...
          // The "panelShow" event is fired asynchronously, after the
          // onShow method call.
          ...
          setTimeout(function() {
            //Reset the disabled menu items:
            $(me._.list.element.$.childNodes)
              .find('.cke_panel_listItem_disabled')
              .removeClass('cke_panel_listItem_disabled');

            // Visually disable the menu items for any disabled commands
            // the items list enumerated here has the command name as the index, and the DOM ID of the corresponding list item element as the value--how convenient!
            $.each(me._.list._.items, function(index, value) {
              if (index === 'paste' && CKEDITOR.currentInstance.newBlock) {
                editor.commands[index].state = CKEDITOR.TRISTATE_DISABLED;
              }
              if (editor.commands[index].state === CKEDITOR.TRISTATE_DISABLED) {
                // if the command is in a disabled state, add disable class to the menu list item element
                $(me._.list.element.$.childNodes)
                  .find('#' + value)
                  .addClass('cke_panel_listItem_disabled');
              }
            });
            // Now it's safe to show the dropdown
            me._.panel.element.setStyle('display', '');

            // save reference to menu for external click handlers (e.g. CEDITOR.topClickHandler()
            if (CKEDITOR !== undefined && CKEDITOR !== null) {
              if (panel !== undefined && panel !== null) {
                CKEDITOR.ui.openMenu = me;
              }
            }
          }, 100);
        ...
        panel.onHide = function(preventOnClose) {
          ...
          //menu has been closed, clear openMenu so no attempt is made to close again
          if (CKEDITOR !== undefined && CKEDITOR !== null) {
            if (CKEDITOR.ui !== undefined && CKEDITOR.ui !== null) {
              CKEDITOR.ui.openMenu = null; //reset
            }
          }
        };
      ...
      add: function(value, html, text, shortcut) {
        this._.items[value] = text || value;
        this._.list.add(value, html, text, shortcut);
      },
      ...
      enable: function() {
        if (this._.state == CKEDITOR.TRISTATE_DISABLED) this.setState(this._.lastState);
      },
